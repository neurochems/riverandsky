...
import es.dfc.riverandsky.entities.ScrollingSprite;
import ddf.minim.AudioOutput;
import ddf.minim.Minim;
import es.dfc.riverandsky.physics.CollisionListener;
import es.dfc.riverandsky.physics.MapBodyManager;
			...
			Tween.to(endItem, SpriteAccessor.ALPHA, 1.4f).target(0).setCallback(new TweenCallback()
				{					
					...
					public void onEvent(int type, BaseTween<?> source) {
						((com.badlogic.gdx.Game) Gdx.app.getApplicationListener()).setScreen(new es.dfc.riverandsky.screens.MainMenu());}   //score screen eventually, unless you want it to be rendered in the black ending??
				}).start(tweenManager);
